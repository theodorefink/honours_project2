...
class TransMIL(nn.Module):
    def __init__(self, n_classes, input_size=1024):
    ...
    def forward(self, x, return_WSI_attn=False):
        ...
        #---->Translayer x1
        attn1 = F.softmax(self.layer1.attn(self.layer1.norm(h), return_attn=True)[1][0, :, :x.shape[1]], dim=1)
        ...
        #---->Translayer x2
        attn2 = F.softmax(self.layer2.attn(self.layer2.norm(h), return_attn=True)[1][0, :, :x.shape[1]], dim=1)
        ...
        #---->cls_token
        cls_token = h[:,0]
        patch_tokens = h[0,1:n+1]

        #---->predict
        logits = F.sigmoid(self._fc2(cls_token)) #[B, n_classes]
        Y_hat = torch.argmax(logits, dim=1)
        Y_prob = F.softmax(logits, dim = 1)
        results_dict['logits'] = logits.squeeze() 
        results_dict['Y_prob'] = Y_prob 
        results_dict['Y_hat'] = Y_hat
        results_dict['A'] = torch.matmul(patch_tokens, cls_tokens.squeeze())
